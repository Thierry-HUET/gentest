...
def _build_numeric_reason(details: dict) -> str:
    failures = [_METRIC_LABEL.get(m, m) for m, info in details.items() if not info.get("ok", True)]
...
def _validate_numeric(
    ...
    details["null_rate"] = {
        "original": p_orig.null_rate, "synthetic": p_synt.null_rate,
        "delta": delta_null, "ok": ok_null,
    }
    if not ok_null:
        all_ok = False
...
def build_report(
    ...
    for pair in constrained_pairs:
        ...
        correlation_reports.append(CorrelationReport(
            ...
            r_original=float(r_orig), r_synthetic=float(r_synt),
            delta=delta, compliant=compliant,
